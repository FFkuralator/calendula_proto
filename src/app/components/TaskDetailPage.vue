...
const cat = computed(() => CATEGORY_CONFIG[props.task.category])
...
const contacts = computed(() =>
  (props.task.contactIds ?? []).map((id) => CONTACTS.find((c) => c.id === id)).filter(Boolean)
)

const docs = computed(() =>
  (props.task.documentIds ?? []).map((id) => DOCS.find((d) => d.id === id)).filter(Boolean)
)

const articles = computed(() =>
  (props.task.relatedArticleIds ?? []).map((id) => KNOWLEDGE_ARTICLES.find((a) => a.id === id)).filter(Boolean)
)
      ...
      <div class="lg:col-span-2 space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6">
          <div class="flex flex-wrap gap-2 mb-4">
            <span :class="['inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border', cat.bg, cat.text, cat.border]">
        ...
        <div v-if="docs.length > 0 || articles.length > 0" class="bg-white rounded-2xl border border-gray-200 p-6">
          ...
          <div class="space-y-2">
            ...
            <div
              v-for="a in articles"
              :key="a!.id"
              class="flex items-center gap-3 p-2.5 rounded-xl border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50 transition-all cursor-pointer"
            >
